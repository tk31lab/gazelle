...
### Architecture
- `src/main.zig`: Entry point for the CLI tool, signal handling (SIGINT), and watcher loop. Also owns `Config` (JSON config file loading + argument overrides) and the LSN position persistence (`Watcher.savePosition` / `loadPosition`).
...
- `src/config_test.zig`: Tests for JSON config file loading and argument overrides.
- `src/persistence_test.zig`: Tests for LSN position save/load.
...
- **Agent Context**: Additional project-specific notes, design documents, or research materials that should not be committed to Git can be placed in the `.gemini/` directory (legacy name, kept for existing design docs). Check this directory for supplemental context when needed.
